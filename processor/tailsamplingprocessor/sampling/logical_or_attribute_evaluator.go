...
func (ae *logicalOrAttributeEvaluator) Evaluate(_ pdata.TraceID, trace *TraceData) (Decision, error) {
	...
	// Returning false from this function will short-circuit the attribute iteration in span.Attributes().Range
	attributeEvaluator := func(key string, value pdata.AttributeValue) bool {
	...
	for _, batch := range batches {
		...
		for i := 0; i < rspans.Len(); i++ {
			...
			resource.Attributes().Range(attributeEvaluator)
			...
			for j := 0; j < ilss.Len(); j++ {
				...
				for k := 0; k < ils.Spans().Len(); k++ {
					span := ils.Spans().At(k)
					span.Attributes().Range(attributeEvaluator)
				}
			}
		}
